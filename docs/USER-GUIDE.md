...
ChatAFL（论文名：**LLMProFuzz**，简称 LPF）是一个**基于 LLM 引导的协议模糊测试框架**，由 AFLNet 扩展而来，发表于《信息网络安全》2025年第12期。

**核心能力：**
- **语法提取**：利用少样本提示工程自动提取协议语法模板
- **种子富集**：基于历史漏洞特征生成高价值初始用例，覆盖边界场景和异常逻辑
- **结构感知变异**：基于协议语法结构选择合适的变异位置，提高有效测试用例生成比例

> **论文引用**：杨立群，李镇，韦超仁，等. 大语言模型引导的协议模糊测试技术研究[J]. 信息网络安全，2025，25(12)：1847-1862.
...
| **ChatAFL-CL2** | 结构感知 + 种子富集 | Ablation 实验 |

> **LLM 干预阈值**： plateau 长度阈值 Max=300（论文实验验证值）
...
| 协议 | Subject | 说明 | 论文评估 |
|------|---------|------|----------|
| **FTP** | PureFTPD, ProFTPD, BFTPD, LightFTP | 文件传输协议 | ProFTPD ✅ |
| **RTSP** | Live555 | 流媒体协议 | Live555 ✅ |
| **SIP** | Kamailio | VoIP 信令协议 | Kamailio ✅ |
| **SMTP** | Exim | 邮件传输协议 | Exim ✅ |
| **HTTP** | Lighttpd | Web 服务器 | Lighttpd ✅ |
| **DAAP** | forked-daapd | iTunes 兼容协议 | ❌ |

> **注**：标记 ✅ 的协议为论文 ProFuzzBench 基准评估中的协议，共5种。
...
> **数据来源**：论文《LLMProFuzz》在 ProFuzzBench 上的评估结果
