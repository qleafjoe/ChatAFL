...
│   ├── afl-fuzz.c              # 主模糊器 (11,465行)
│   ├── chat-llm.c              # LLM交互层 (1,576行)
│   ├── chat-llm.h              # LLM接口头文件 (119行)
│   ├── llm-validator.c         # 校验框架 (613行)
│   ├── llm-validator.h         # 校验头文件 (144行)
│   ├── config.h                # 关键常量定义
│   ├── env.sh                  # 环境变量配置
│   └── Makefile                # 构建系统
├── ChatAFL-V0/                 # 消融变体V0：无校验基线
│   ├── afl-fuzz.c              # 旧版源码 (10,946行，无校验代码)
│   └── env.sh                  # VALIDATION=0, FEEDBACK=0
├── ChatAFL-V1/                 # 消融变体V1：仅格式校验
...
| 特性 | V0 | V1 | V2 | ChatAFL |
|------|----|----|----|---------|
| **源码** | 独立 afl-fuzz.c (10,946行) | 与ChatAFL相同 (11,465行) | 与ChatAFL相同 | 完整版 |
...
**V0 vs ChatAFL**:
- V0 的 `afl-fuzz.c` 是独立的旧版代码（10,946行），缺少 `#include "llm-validator.h"` 和所有校验相关代码
- V0 的 `afl-fuzz.c` 约 500 行差异，主要是校验逻辑和反馈重试代码

**V1/V2 vs ChatAFL**:
- `afl-fuzz.c` 完全相同（字节级一致）
- 差异仅在 `env.sh` 配置文件
- 校验代码编译进二进制，但通过环境变量在运行时控制

### 8.5 消融实验的设计目标

| 变体 | 消融目标 | 对比关系 |
|------|----------|----------|
| **V0** | LLM无校验基线 | V0 vs V1 → 格式校验的贡献 |
| **V1** | 仅格式校验 | V1 vs V2 → 语法+上下文校验的贡献 |
| **V2** | 完整校验（无post-gain） | V2 vs ChatAFL → Post-gain归因+显式feedback的贡献 |
| **ChatAFL** | 完整系统 | 所有特性开启 |
