...
struct queue_entry
{
  ...
  u8 llm_seed_priority;    /* Lower priority for soft-accepted LLM seeds */
...
char *protocol_name;
// TR4 fixed ablation profile: Hard Validation + feedback retry + Soft Validation.
...
#define TR_SOFT_ACCEPT_RATE_FORMAT 0
#define TR_SOFT_ACCEPT_RATE_GRAMMAR 5
#define TR_SOFT_ACCEPT_RATE_CONTEXT 30
#define TR_SOFT_SEED_PRIORITY 2
...
static u8 current_llm_seed_priority = 0;
...
static u8 should_soft_accept_llm_result(llm_validation_result_t result)
{
  ...
  case LLM_VALID_FORMAT_FAIL:
    return UR(100) < TR_SOFT_ACCEPT_RATE_FORMAT;
  case LLM_VALID_GRAMMAR_FAIL:
    return UR(100) < TR_SOFT_ACCEPT_RATE_GRAMMAR;
  case LLM_VALID_CONTEXT_FAIL:
    return UR(100) < TR_SOFT_ACCEPT_RATE_CONTEXT;
  default:
    return 0;
  }
}
...
{
  char *pattern_dir = alloc_printf("%s/protocol-grammars", out_dir);
  DIR *d = opendir(pattern_dir);
  if (!d) { ck_free(pattern_dir); return 0; }

  int loaded = 0;
  struct dirent *ent;
  while ((ent = readdir(d)) != NULL) {
    if (strncmp(ent->d_name, "pattern-", 8) != 0) continue;

    char *path = alloc_printf("%s/%s", pattern_dir, ent->d_name);
    FILE *f = fopen(path, "r");
    if (!f) { ck_free(path); continue; }

    char header_buf[2048] = {0};
    char fields_buf[8192] = {0};
    if (!fgets(header_buf, sizeof(header_buf), f) ||
        !fgets(fields_buf, sizeof(fields_buf), f)) {
      fclose(f); ck_free(path); continue;
    }
    fclose(f);

    /* Strip trailing newlines */
    header_buf[strcspn(header_buf, "\r\n")] = '\0';
    fields_buf[strcspn(fields_buf, "\r\n")] = '\0';

    if (header_buf[0] == '\0' || fields_buf[0] == '\0') {
      ck_free(path); continue;
    }
    ...
    patterns[0] = pcre2_compile((PCRE2_SPTR)header_buf, PCRE2_ZERO_TERMINATED,
                                0, &errornumber, &erroroffset, NULL);
    patterns[1] = pcre2_compile((PCRE2_SPTR)fields_buf, PCRE2_ZERO_TERMINATED,
                                0, &errornumber, &erroroffset, NULL);

    if (patterns[0] && patterns[1]) {
      pcre2_jit_compile(patterns[0], PCRE2_JIT_COMPLETE);
      pcre2_jit_compile(patterns[1], PCRE2_JIT_COMPLETE);
  ...
  closedir(d);
  ck_free(pattern_dir);
  ACTF("Loaded %d cached grammar patterns from disk", loaded);
  return loaded;
}
...
static void add_to_queue(u8 *fname, u32 len, u8 passed_det)
{
  ...
  q->is_initial_seed = 0;
  q->llm_seed_priority = current_llm_seed_priority;
  if (!q->llm_seed_priority && fname && strstr((char *)fname, "/soft_enriched_"))
    q->llm_seed_priority = TR_SOFT_SEED_PRIORITY;
  ...
  for (int i = 0; i < nl_cnt; i++)
  {
    ...
    for(int i = 0;i < kv_size(message_subsets);i++) {
      ...
      // Try enriching the sequence
        ...
        char *enriched_file_name = malloc(strlen(nl_file_name) + 32);
        if (soft_accepted_llm_seed) {
          strcpy(enriched_file_name, "soft_enriched_");
          sprintf(enriched_file_name + 14, "%d_", i);
        } else {
...
static void maybe_delete_out_dir(void)
{
  ...
  /* Delete the old protocol-grammars folder (skip when reusing cached patterns) */
...
static u32 calculate_score(struct queue_entry *q)
{
  ...
  if (q->llm_seed_priority > 0 && q->llm_seed_priority < 10)
  {
    perf_score = (perf_score * q->llm_seed_priority) / 10;
    if (!perf_score)
      perf_score = 1;
  }
      ...
      if (!empty)
      {
        ...
        if (stall_message != NULL && afl_llm_validation) {
          ...
          if (stall_message != NULL)
          {
            ...
            if (soft_accepted_stall_seed)
              current_llm_seed_priority = TR_SOFT_SEED_PRIORITY;
            u8 should_abort = common_fuzz_stuff(argv, stall_message, strlen(stall_message));
            current_llm_seed_priority = 0;
...
int main(int argc, char **argv)
{
  ...
  afl_llm_validation = TR_LLM_VALIDATION;
  afl_llm_validation_permissive = TR_LLM_VALIDATION_PERMISSIVE;
  afl_llm_validation_strict = TR_LLM_VALIDATION_STRICT;
  afl_llm_post_gain = TR_LLM_POST_GAIN;
  afl_llm_feedback = TR_LLM_FEEDBACK;
  afl_llm_feedback_max_retries = TR_LLM_FEEDBACK_MAX_RETRIES;
  afl_llm_skip_startup = TR_LLM_SKIP_STARTUP;
  ...
  if (protocol_selected)
  {
    ...
    if (!afl_llm_skip_startup) {
      ...
      int n = load_cached_grammars();
      WARNF("AFL_LLM_SKIP_STARTUP=1: loaded %d cached patterns, skipping LLM calls", n);
    }
  }
  read_testcases();
  load_auto();
