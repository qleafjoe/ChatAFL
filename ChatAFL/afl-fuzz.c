...
struct queue_entry
{
  ...
  u8 is_llm_seed;          /* Whether this seed was generated by LLM */
  u32 llm_priority;        /* LLM seed priority (higher = more important) */
};
...
u8 afl_llm_skip_startup = 0;          // AFL_LLM_SKIP_STARTUP=0/1
u8 last_llm_exec_fault = 0;
u8 last_llm_exec_interesting = 0;
u64 last_llm_exec_us = 0;

/* Seed deduplication hash table */
static u8 *seed_hash_table = NULL;
static u32 seed_hash_table_size = 0;

/* Flag to mark next added seed as LLM-generated */
static u8 mark_next_seed_as_llm = 0;
static u32 next_seed_llm_priority = 0;

/* Dynamic mutation stage weights */
#define MUTATION_STAGE_COUNT 17
static u32 stage_weights[MUTATION_STAGE_COUNT] = {
    100, /* STAGE_FLIP1 */
    100, /* STAGE_FLIP2 */
    100, /* STAGE_FLIP4 */
    100, /* STAGE_FLIP8 */
...
/* Initialize seed hash table for deduplication */
static void init_seed_hash_table(u32 size) {
  seed_hash_table_size = size;
  seed_hash_table = ck_alloc(size * sizeof(u8));
}

/* Check if seed data is a duplicate (by content hash). Returns 1 if duplicate, 0 if new. */
...
/* Adjust stage weights based on coverage feedback */
static void adjust_stage_weights(void) {
  for (int i = 0; i < MUTATION_STAGE_COUNT; i++) {
    if (stage_cycles[i] > 100) {
      u32 gain = (u32)(stage_finds[i] * 100 / stage_cycles[i]);

      if (gain > 10) {
        stage_weights[i] = 150; /* High gain, increase weight */
      } else if (gain < 1) {
        stage_weights[i] = 50;  /* Low gain, decrease weight */
      } else {
        stage_weights[i] = 100; /* Normal weight */
      }
    }
  }
}
...
struct queue_entry *choose_seed(u32 target_state_id, u8 mode)
{
  ...
  if (k != kh_end(khms_states))
  {
    ...
    /* LLM seed priority: prefer LLM-generated seeds that haven't been fuzzed yet */
    {
      u32 i;
      for (i = 0; i < state->seeds_count; i++) {
        struct queue_entry *candidate = state->seeds[i];
        if (candidate->is_llm_seed && candidate->llm_priority > 0 &&
            !was_fuzzed_map[get_state_index(target_state_id)][candidate->index]) {
          return candidate;
        }
      }
    }
...
static void add_to_queue(u8 *fname, u32 len, u8 passed_det)
{
  ...
  mark_next_seed_as_llm = 0;
  next_seed_llm_priority = 0;
  ...
  for (int i = 0; i < nl_cnt; i++)
  {
    ...
    for(int i = 0;i < kv_size(message_subsets);i++) {
      ...
      // Try enriching the sequence
        ...
        if (seed_hash_table && is_seed_duplicate((const u8 *)unescaped_client_requests, strlen(unescaped_client_requests))) {
...
static void read_testcases(void)
{
  ...
  for (i = 0; i < nl_cnt; i++)
  {
    ...
    /* Mark LLM-generated seeds (enriched_ files) */
      ...
      if (!empty)
      {
        ...
        if (stall_message != NULL && afl_llm_validation) {
          ...
          if (stall_message != NULL)
          {
            ...
            /* Mark stall-generated seed as LLM seed */
...
int main(int argc, char **argv)
{
  ...
  afl_llm_skip_startup = env_flag_enabled("AFL_LLM_SKIP_STARTUP");
  ...
  /* Initialize seed deduplication hash table (64K entries) */
  init_seed_hash_table(65536);
  ...
  if (state_aware_mode)
  {
    ...
    while (1)
    {
      ...
      /* Adjust mutation stage weights based on coverage feedback */
      adjust_stage_weights();
