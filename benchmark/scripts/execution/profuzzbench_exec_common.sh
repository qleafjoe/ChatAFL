...
for i in $(seq 1 $RUNS); do
  id=$(docker run --cpus=1 -e LLM_URL="${LLM_URL}" -e LLM_TOKEN="${LLM_TOKEN}" -e LLM_MODEL="${LLM_MODEL}" -d -it $DOCIMAGE /bin/bash -c "cd ${WORKDIR} && if [ -f ${WORKDIR}/${FUZZER}/env.sh ]; then . ${WORKDIR}/${FUZZER}/env.sh; fi && run ${FUZZER} ${OUTDIR} '${OPTIONS}' ${TIMEOUT} ${SKIPCOUNT}")
