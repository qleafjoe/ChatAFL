...
for i in $(seq 1 $RUNS); do
  id=$(docker run --cpus=1 -e LLM_URL="${LLM_URL}" -e LLM_TOKEN="${LLM_TOKEN}" -e LLM_MODEL="${LLM_MODEL}" -e AFL_LLM_VALIDATION="${AFL_LLM_VALIDATION}" -e AFL_LLM_VALIDATION_PERMISSIVE="${AFL_LLM_VALIDATION_PERMISSIVE}" -e AFL_LLM_VALIDATION_STRICT="${AFL_LLM_VALIDATION_STRICT}" -e AFL_LLM_POST_GAIN="${AFL_LLM_POST_GAIN}" -e AFL_LLM_FEEDBACK="${AFL_LLM_FEEDBACK}" -e AFL_LLM_FEEDBACK_MAX_RETRIES="${AFL_LLM_FEEDBACK_MAX_RETRIES}" -e AFL_LLM_SKIP_STARTUP="${AFL_LLM_SKIP_STARTUP}" -d -it $DOCIMAGE /bin/bash -c "cd ${WORKDIR} && if [ -f ${WORKDIR}/${FUZZER}/env.sh ]; then . ${WORKDIR}/${FUZZER}/env.sh; fi && run ${FUZZER} ${OUTDIR} '${OPTIONS}' ${TIMEOUT} ${SKIPCOUNT}")
