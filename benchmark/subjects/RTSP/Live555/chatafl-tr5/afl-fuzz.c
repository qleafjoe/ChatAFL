...
/* ---- TR5 fixed ablation profile: Contextual Stall-Only Soft Validation ---- */
#define TR_LLM_VALIDATION 1
#define TR_LLM_VALIDATION_PERMISSIVE 0
#define TR_LLM_VALIDATION_STRICT 1
#define TR_LLM_POST_GAIN 0
#define TR_LLM_FEEDBACK 1
#define TR_LLM_FEEDBACK_MAX_RETRIES 3
#define TR_LLM_SKIP_STARTUP 0

/* TR5-specific compile-time constants */
#define TR5_ENABLE_STALL_SOFT_VALIDATION 1
#define TR5_ENABLE_ENRICHMENT_SOFT_VALIDATION 0
#define TR5_REQUIRE_REVALIDATION_AFTER_CONTEXTUALIZE 1

u8 afl_llm_validation = TR_LLM_VALIDATION;
u8 afl_llm_validation_permissive = TR_LLM_VALIDATION_PERMISSIVE;
u8 afl_llm_validation_strict = TR_LLM_VALIDATION_STRICT;
u8 afl_llm_post_gain = TR_LLM_POST_GAIN;
u8 afl_llm_feedback = TR_LLM_FEEDBACK;
u32 afl_llm_feedback_max_retries = TR_LLM_FEEDBACK_MAX_RETRIES;
u8 afl_llm_skip_startup = TR_LLM_SKIP_STARTUP;
...
/* ---- TR5: Protocol-Generic Runtime Context for Stall Soft Validation ---- */
typedef struct {
  /* RTSP fields */
  u32 last_cseq;
  char *last_session;     /* dynamically allocated, must be freed */
  char *last_transport;   /* dynamically allocated, must be freed */
  char *last_uri;         /* dynamically allocated, must be freed */
  char *last_method;      /* dynamically allocated, must be freed */
  u8 has_session;
  u8 has_transport;
  u8 has_uri;
  u8 has_cseq;
  u8 has_method;
  /* FTP slots (not implemented in v1) */
  u8 ftp_has_user;
  u8 ftp_has_pass;
  u8 ftp_is_authed;
  /* HTTP slots (not implemented in v1) */
  u8 http_has_content_length;
  u8 http_has_host;
} protocol_runtime_context_t;

/* ---- TR5 Validation Helpers ---- */

/* Case-insensitive search for a header at the beginning of a line */
...
/* ---- TR5: Protocol-Generic Runtime Context Extraction ---- */
...
/* Extract runtime context from stall interaction history.
 * Parses recent history to recover Session, CSeq, Transport, URI, and last method.
 * This is the source of truth for candidate repair and accept/reject decisions. */
    ...
    protocol_runtime_context_t *ctx)
{
  ...
  /* Parse the history from end to start to get the most recent values */
  ...
  while (line && *line) {
    ...
    if (line_len >= sizeof(line_buf) - 1) line_len = sizeof(line_buf) - 2;
    ...
    /* Extract CSeq */
    if (strncasecmp(line_buf, "CSeq:", 5) == 0) {
      u32 cseq_val = (u32)atoi(line_buf + 5);
      if (cseq_val > max_cseq) {
        max_cseq = cseq_val;
        ctx->last_cseq = cseq_val;
        ctx->has_cseq = 1;
      }
    }

    /* Extract Session */
    if (strncasecmp(line_buf, "Session:", 8) == 0) {
      const char *val = line_buf + 8;
      while (*val == ' ' || *val == '\t') val++;
      if (*val) {
        if (ctx->last_session) free(ctx->last_session);
        ctx->last_session = strdup(val);
        ctx->has_session = 1;
      }
    }

    /* Extract Transport */
    if (strncasecmp(line_buf, "Transport:", 10) == 0) {
      const char *val = line_buf + 10;
      while (*val == ' ' || *val == '\t') val++;
      if (*val) {
        if (ctx->last_transport) free(ctx->last_transport);
        ctx->last_transport = strdup(val);
        ctx->has_transport = 1;
      }
    }

    /* Extract URI from request line (METHOD URI VERSION) */
    if (strstr(line_buf, "RTSP/1.0") || strstr(line_buf, "rtsp://")) {
      char method_buf[64] = {0};
      char uri_buf[2048] = {0};
      if (sscanf(line_buf, "%63s %2047s", method_buf, uri_buf) == 2) {
...
/* Check if the candidate message is for a transition-critical RTSP method.
 * Only SETUP, PLAY, PAUSE, TEARDOWN are considered transition-critical. */
    ...
    const protocol_runtime_context_t *ctx)
{
  ...
  /* Extract method from the candidate */
  char method_buf[64] = {0};
  if (sscanf(candidate, "%63s", method_buf) != 1) return 0;

  /* Transition-critical methods */
...
/* Try to contextualize a candidate by patching missing fields from history.
 * Returns a newly allocated patched candidate, or NULL if patching fails.
 * Caller must ck_free() the returned string. */
    ...
    const protocol_runtime_context_t *ctx)
{
  ...
  /* We need to patch missing fields. Strategy:
   * - If no Session header and ctx has one, inject it
   * - If no CSeq header and ctx has one, inject it
   * - If no Transport header (for SETUP) and ctx has one, inject it
   * - If URI is "*" or empty and ctx has one, replace it */

  size_t cand_len = strlen(candidate);
  /* Allocate extra space for injected headers */
  size_t buf_size = cand_len + 1024;
  char *patched = ck_alloc(buf_size);
  if (!patched) return NULL;

  /* Copy candidate, checking for existing headers */
  int has_session = 0;
  int has_cseq = 0;
  int has_transport = 0;
  int has_uri = 1; /* assume present unless we detect otherwise */

  /* Check headers in the candidate */
  const char *check = candidate;
  while (check && *check) {
    const char *nl = strchr(check, '\n');
    size_t line_len = nl ? (size_t)(nl - check) : strlen(check);

    if (line_len > 8 && strncasecmp(check, "Session:", 8) == 0) has_session = 1;
    if (line_len > 5 && strncasecmp(check, "CSeq:", 5) == 0) has_cseq = 1;
    if (line_len > 10 && strncasecmp(check, "Transport:", 10) == 0) has_transport = 1;

    /* Check for bad URI */
    if (strstr(check, "RTSP/1.0")) {
      char method_buf[64] = {0};
      char uri_buf[2048] = {0};
      if (sscanf(check, "%63s %2047s", method_buf, uri_buf) == 2) {
        if (strcmp(uri_buf, "*") == 0 || uri_buf[0] == '\0') {
          has_uri = 0;
        }
      }
    }
  ...
  /* Now reconstruct the message with patched fields */
  char *write_ptr = patched;
  const char *read_ptr = candidate;
  int injected_session = 0;
  int injected_cseq = 0;
  int injected_transport = 0;
  int patched_uri = 0;

  while (read_ptr && *read_ptr) {
    const char *nl = strchr(read_ptr, '\n');
    size_t line_len = nl ? (size_t)(nl - read_ptr) : strlen(read_ptr);

    /* Check if this is the request line */
    if (strstr(read_ptr, "RTSP/1.0") && !patched_uri) {
      if (!has_uri && ctx->has_uri && ctx->last_uri) {
        /* Patch the URI */
        char method_buf[64] = {0};
        sscanf(read_ptr, "%63s", method_buf);
        int w = snprintf(write_ptr, buf_size - (write_ptr - patched),
                         "%s %s RTSP/1.0\r\n", method_buf, ctx->last_uri);
        write_ptr += w;
        patched_uri = 1;
        read_ptr = nl ? nl + 1 : read_ptr + line_len;
        continue;
      }
    }
    ...
    *write_ptr++ = '\n';
    ...
    /* After the request line, inject missing headers before the first header line */
    if (!patched_uri && write_ptr > patched + 2) {
      patched_uri = 1; /* mark that we've passed the request line */
    }

    /* Inject headers at the end of existing headers (before \r\n\r\n) */
    if (*read_ptr == '\r' || *read_ptr == '\n' || *read_ptr == '\0') {
      /* We're at the end of headers - inject missing ones */
      if (!has_cseq && ctx->has_cseq && !injected_cseq) {
        int w = snprintf(write_ptr, buf_size - (write_ptr - patched),
                         "CSeq: %u\r\n", ctx->last_cseq);
        write_ptr += w;
        injected_cseq = 1;
      }
      if (!has_session && ctx->has_session && ctx->last_session && !injected_session) {
        int w = snprintf(write_ptr, buf_size - (write_ptr - patched),
                         "Session: %s\r\n", ctx->last_session);
        write_ptr += w;
        injected_session = 1;
      }
      if (!has_transport && ctx->has_transport && ctx->last_transport && !injected_transport) {
        /* Only inject Transport for SETUP */
        char first_line[256] = {0};
        const char *first_nl = strchr(candidate, '\n');
        size_t fl_len = first_nl ? (size_t)(first_nl - candidate) : strlen(candidate);
        if (fl_len < sizeof(first_line)) {
          memcpy(first_line, candidate, fl_len);
          first_line[fl_len] = '\0';
          if (strncasecmp(first_line, "SETUP", 5) == 0) {
            int w = snprintf(write_ptr, buf_size - (write_ptr - patched),
                             "Transport: %s\r\n", ctx->last_transport);
            write_ptr += w;
            injected_transport = 1;
          }
        }
      }
    }
  ...
  *write_ptr = '\0';
  ...
  /* If we couldn't inject what was needed, return NULL */
  int needs_session = 0;
  int needs_cseq = !has_cseq;
  ...
  if (fl_len < sizeof(first_line)) {
    ...
    if (strncasecmp(first_line, "SETUP", 5) == 0) {
      ...
      needs_transport = !has_transport;
      needs_session = 0;  /* SETUP does not require Session */
    } else {
      /* PLAY/PAUSE/TEARDOWN need CSeq and Session, but NOT Transport */
      needs_session = !has_session;
      needs_transport = 0;
    }
  }

  if ((needs_session && !injected_session) ||
      (needs_cseq && !injected_cseq) ||
      (needs_transport && !injected_transport)) {
    ck_free(patched);
    return NULL;
  }
...
/* TR5 Soft Accept Policy for RTSP.
 * Returns 1 if the candidate can be soft-accepted (recovered), 0 otherwise.
 * Only LLM_VALID_CONTEXT_FAIL is eligible for soft recovery.
 * FORMAT_FAIL and GRAMMAR_FAIL are always rejected.
 * Note: ctx can be NULL when called for eligibility check before context extraction. */
static int can_soft_accept_candidate(
    const char *protocol,
    llm_validation_result_t validation_result,
    const char *candidate,
    const protocol_runtime_context_t *ctx)
{
  if (!protocol || !candidate) return 0;
  if (!TR5_ENABLE_STALL_SOFT_VALIDATION) return 0;

  /* Only CONTEXT_FAIL is eligible for soft recovery */
  if (validation_result != LLM_VALID_CONTEXT_FAIL) return 0;

  /* Must be a transition-critical method (ctx can be NULL here) */
  if (!is_transition_critical_candidate(protocol, candidate, ctx)) return 0;

  /* If ctx is NULL, we cannot check context availability - return 1 to allow
   * caller to extract context and re-check */
  if (!ctx) return 1;

  /* For RTSP: check method-specific context availability */
  if (strcmp(protocol, "RTSP") == 0) {
    char method_buf[64] = {0};
    if (sscanf(candidate, "%63s", method_buf) != 1) return 0;
    ...
    /* SETUP needs CSeq + Transport; PLAY/PAUSE/TEARDOWN need CSeq + Session */
    if (strcasecmp(method_buf, "SETUP") == 0) {
      if (!ctx->has_cseq || !ctx->has_transport) return 0;
    } else {
      if (!ctx->has_cseq || !ctx->has_session) return 0;
    }
  } else {
    /* Generic: must have at least CSeq or Session */
    if (!ctx->has_cseq && !ctx->has_session) return 0;
  }
    ...
    const protocol_runtime_context_t *ctx)
{
  ...
  /* Default: CSeq is sufficient */
  return 1;
}

/* Load cached grammar patterns from protocol-grammars/pattern-* files.
   Returns the number of patterns loaded, or 0 if none found. */
static int load_cached_grammars(void)
{
  ...
  for (int i = 0; i < nl_cnt; i++)
  {
    ...
    for(int i = 0;i < kv_size(message_subsets);i++) {
      ...
      // Try enriching the sequence
        ...
        /* TR5: Strict validation for enrichment (no soft accept) */
  ...
  if (uninteresting_times >= UNINTERESTING_THRESHOLD && chat_times < CHATTING_THRESHOLD)
  {
    ...
    if (responses_temp != NULL)
    {
      ...
      if (!empty)
      {
        ...
        /* TR5: Use artifact writer for stall interactions */
        static u32 stall_call_id_seq = 0;
        u32 stall_call_id = ++stall_call_id_seq;

        write_llm_text_artifact("stall-interactions", "prompt",
                                stall_call_id, stall_prompt);

        if (stall_response == NULL)
          goto free_stall;

        write_llm_text_artifact("stall-interactions", "response",
                                stall_call_id, stall_response);
        ...
        /* format_request_message frees stall_message input, returns ck_alloc'd string */
        stall_message = format_request_message(stall_message);

        if (stall_message != NULL)
        {
          write_llm_text_artifact("stall-interactions", "candidate",
                                  stall_call_id, stall_message);

          /* Snapshot execution metrics before candidate execution */
          u32 queued_before = queued_paths;
          u32 state_before = state_ids_count;
          u32 edge_before = ipsm ? agnedges(ipsm) : 0;

          char *execute_message = stall_message; /* may be replaced by contextualized version */
          int soft_accepted = 0;

          /* ---- TR5: Validation + Soft Accept for stall ---- */
          if (afl_llm_validation)
          {
            llm_validation_record_t record;
            protocol_context_t ctx;
            ...
            record.llm_call_id = stall_call_id;
            init_protocol_context(&ctx);
            ...
            /* TR5: Populate analysis fields */
            record.original_validation_result = record.result;
            record.post_contextualize_validation_result = record.result; /* updated below if contextualized */
            record.is_transition_critical = is_transition_critical_candidate(
                protocol_name, stall_message, NULL) ? 1 : 0;
            record.runtime_ctx_available = 0;
            record.recovered_field_count = 0;
            record.soft_accept_reason[0] = '\0';
            record.recovered_fields[0] = '\0';

            if (record.result != LLM_VALID_OK && !afl_llm_validation_permissive)
            {
              /* ---- TR5 Soft Accept: try contextualize for CONTEXT_FAIL ---- */
              int recovered = 0;

              if (TR5_ENABLE_STALL_SOFT_VALIDATION &&
                  can_soft_accept_candidate(protocol_name, record.result,
                                            stall_message, NULL))
              {
                /* Extract real runtime context from interaction history */
                protocol_runtime_context_t rt_ctx;
                extract_runtime_context_from_history(protocol_name, history, &rt_ctx);
                record.runtime_ctx_available = has_sufficient_context_for_method(
                    protocol_name, stall_message, &rt_ctx) ? 1 : 0;

                if (has_sufficient_context_for_method(protocol_name, stall_message, &rt_ctx))
                {
                  char *contextualized = contextualize_candidate(
                      protocol_name, stall_message, &rt_ctx);

                  if (contextualized)
                  {
                    /* Revalidate the patched candidate */
                    protocol_context_t ctx2;
                    init_protocol_context(&ctx2);
                    llm_validation_result_t reval =
                        validate_llm_message_with_mode(protocol_name,
                                                       LLM_STAGE_STALL,
                                                       contextualized,
                                                       &ctx2,
                                                       validation_mode);
                    record.post_contextualize_validation_result = reval;

                    if (reval == LLM_VALID_OK)
                    {
                      /* Soft accept: use the contextualized message */
                      execute_message = contextualized;
                      soft_accepted = 1;
                      recovered = 1;
                      ...
                      /* Count recovered fields by checking what contextualize actually injected */
                      u8 recovered_count = 0;
                      char recovered_buf[128] = {0};
                      if (!find_header(stall_message, "session:") && find_header(contextualized, "session:")) {
                        recovered_count++;
                        if (recovered_buf[0]) strncat(recovered_buf, "|", sizeof(recovered_buf) - strlen(recovered_buf) - 1);
                        strncat(recovered_buf, "Session", sizeof(recovered_buf) - strlen(recovered_buf) - 1);
                      }
                      if (!find_header(stall_message, "cseq:") && find_header(contextualized, "cseq:")) {
                        recovered_count++;
                        if (recovered_buf[0]) strncat(recovered_buf, "|", sizeof(recovered_buf) - strlen(recovered_buf) - 1);
                        strncat(recovered_buf, "CSeq", sizeof(recovered_buf) - strlen(recovered_buf) - 1);
                      }
                      if (!find_header(stall_message, "transport:") && find_header(contextualized, "transport:")) {
                        recovered_count++;
                        if (recovered_buf[0]) strncat(recovered_buf, "|", sizeof(recovered_buf) - strlen(recovered_buf) - 1);
                        strncat(recovered_buf, "Transport", sizeof(recovered_buf) - strlen(recovered_buf) - 1);
                      }
                      record.recovered_field_count = recovered_count;
                      strncpy(record.recovered_fields, recovered_buf, sizeof(record.recovered_fields) - 1);
                      strncpy(record.soft_accept_reason, "context_fail_transition_critical", sizeof(record.soft_accept_reason) - 1);

                      write_llm_text_artifact("stall-interactions",
                                              "contextualized",
                                              stall_call_id, contextualized);
                      ACTF("TR5 soft-accepted stall candidate #%u "
                           "(contextualized with CSeq=%u Session=%s)",
                           stall_call_id,
                           rt_ctx.last_cseq,
                           rt_ctx.last_session ? rt_ctx.last_session : "none");
                    }
                    else
                    {
                      /* Revalidation failed; fall through to feedback */
                      ck_free(contextualized);
                      strncpy(record.soft_accept_reason, "revalidation_failed", sizeof(record.soft_accept_reason) - 1);
                    }
                  }
                  else
                  {
                    strncpy(record.soft_accept_reason, "contextualize_failed", sizeof(record.soft_accept_reason) - 1);
                  }
                }
                else
                {
                  strncpy(record.soft_accept_reason, "no_runtime_context", sizeof(record.soft_accept_reason) - 1);
                }
                free_runtime_context(&rt_ctx);
              }
              else
              {
                strncpy(record.soft_accept_reason, "not_eligible", sizeof(record.soft_accept_reason) - 1);
              }

              /* ---- Feedback retry if soft accept did not recover ---- */
              if (!recovered && afl_llm_feedback)
              {
                char *retry_msg = llm_feedback_retry_stall(
                    protocol_name, stall_message, record.result,
                    validation_mode, afl_llm_feedback_max_retries);

                if (retry_msg)
                {
                  /* Validate the feedback-recovered message */
                  protocol_context_t ctx3;
                  init_protocol_context(&ctx3);
                  llm_validation_result_t retry_result =
                      validate_llm_message_with_mode(protocol_name,
                                                     LLM_STAGE_STALL,
                                                     retry_msg,
                                                     &ctx3,
                                                     validation_mode);
                  if (retry_result == LLM_VALID_OK)
                  {
                    ck_free(stall_message);
                    stall_message = retry_msg;
                    execute_message = stall_message;
                    record.result = LLM_VALID_OK;
                    fill_validation_reason(&record, record.result,
                                           "stall_feedback_accept");
                    recovered = 1;
                  }
                  else
                  {
              ...
              /* If still not recovered, log and reject */
              if (!recovered)
              {
                log_llm_validation_record(&record);
                ck_free(stall_message);
          ...
          if (common_fuzz_stuff(argv, execute_message, strlen(execute_message)))
          {
            splicing_with = -1;
            ...
            if (afl_llm_validation)
            {
              llm_validation_record_t post_rec;
              init_validation_record_common(&post_rec, LLM_STAGE_STALL,
                                            strlen(execute_message));
              post_rec.llm_call_id = stall_call_id;
              fill_post_execution_record(&post_rec, queued_before,
                                         state_before, edge_before);
              if (soft_accepted)
                fill_validation_reason(&post_rec, post_rec.result,
                                       "stall_soft_accept_exec");
              else
                fill_validation_reason(&post_rec, post_rec.result,
                                       "stall_exec");
              log_llm_validation_record(&post_rec);
            }

            /* Free contextualized message if it was used as execute_message */
            if (soft_accepted && execute_message != stall_message)
              ck_free(execute_message);
            ck_free(stall_message);
            delete_kl_messages(kl_messages);
            return ret_val;
          }

          /* Post-execution logging even on calibration failure */
          if (afl_llm_validation)
          {
            llm_validation_record_t post_rec;
            init_validation_record_common(&post_rec, LLM_STAGE_STALL,
...
EXP_ST void setup_dirs_fds(void)
{
  ...
  /* TR5: Enrichment interaction artifacts */
  tmp = alloc_printf("%s/enrichment-interactions", out_dir);
  if (mkdir(tmp, 0700))
    PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  /* TR5: Validation log directory */
  tmp = alloc_printf("%s/llm-validation", out_dir);
  if (mkdir(tmp, 0700))
    PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);
...
int main(int argc, char **argv)
{
  ...
  /* TR5: Initialize validation CSV logs */
  if (afl_llm_validation)
